...
bool isFile(const char *fileName)
{
    ...
    MultiByteToWideChar(CP_ACP, 0, fileName, -1, wideFileName, size);

    DWORD att = GetFileAttributes(wideFileName);
		...
		const char *pattern)
{
    ...
    int size = MultiByteToWideChar(CP_ACP, 0, filePattern.cstr(), -1, NULL, 0);
    wchar_t* wideFilePattern = new wchar_t[size];
    MultiByteToWideChar(CP_ACP, 0, filePattern.cstr(), -1, wideFilePattern, size);

    WIN32_FIND_DATAW findData;
    HANDLE handle = FindFirstFileW(wideFilePattern, &findData);
    ...
    if (handle != INVALID_HANDLE_VALUE)
    {
        do {
                ...
                (t == ftDirectory && isDir))
            {
                ...
                wcstombs(mbFileName, wFileName, sizeof(mbFileName));
                entries.pushBack(mbFileName);
            }
