...
void my_cpuid(uint32_t CPUInfo[4], uint32_t infoType) {
    // Thay thế hàm __cpuid bằng cách sử dụng inline assembly
    asm volatile (
        "cpuid"
...
void System::init()
{
    ...
    my_cpuid(CPUInfo, 0x80000000);
	unsigned int nExIds = CPUInfo[0];

	if(nExIds >= 0x80000006) {
        my_cpuid(CPUInfo, 0x80000006);
